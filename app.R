        ...
        tabPanel(
            ...
            sidebarLayout(
                ...
                mainPanel(
                    tabsetPanel(
                        tabPanel("Summary", 
                                 textOutput("c2_sample_text1"),
                                 # textOutput("c2_sample_text2"),
                                 # textOutput("c2_sample_text3"),
                ...
                mainPanel(
                    tabsetPanel(
                        ...
                        tabPanel(
                            ...
                            sidebarLayout(
                                sidebarPanel(
                                    HTML(paste(rep("<p>A volcano plot can be generated</p>"),
                                               collapse = "")),
            ...
            sidebarLayout(
                sidebarPanel(
                    fileInput("c4_fileinput1", "Load normalized count matrix data",
                              accept = ".csv", placeholder = "norm_count_matrix.csv"),
                    fileInput("c4_fileinput2", "Load sample information data",
                              accept = ".csv", placeholder = "sample_info.csv"),
                ),
                mainPanel(
                    sidebarLayout(
...
server <- function(input, output, session) {
    ...
    output$c2_table <- renderTable({
        ...
        output$c2_sample_text1 <- renderText({ 
          paste(c("Total samples in dataset ", dim(m)[2]),collapse="")
        })
        # output$c2_sample_text2 <- renderText({ 
        #   paste(c("Filter criteria 1: Genes with at least ", c2.slider_ptile()/100, " percentile variance"),collapse="")
        # })
        # output$c2_sample_text3 <- renderText({ 
        #   paste(c("Filter criteria 2: Genes with at least ", c2.slider_nz_sampl(), " non-zero samples"), collapse="")
        # })
        return(tbl)
    })
    ...
    output$c4_plot <- renderPlot({
        ...
        if(gene=="") {
          output$c4_error <- renderText({
            paste(c("Please enter a gene name and click on Plot"))
          })
          return(NULL)
        } else if(gene %in% genelist == FALSE) {
            output$c4_error <- renderText({
